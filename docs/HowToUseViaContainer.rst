...
For the latest InterProScan release (5.71-102.0), data can be downloaded with the following command:
...
::
    ...
    md5sum -c interproscan-data-5.71-102.0.tar.gz.md5
...
The data will be extracted to interproscan-5.71-102.0/data.
...
::
    ...
    docker run --rm \
        -v $PWD/interproscan-5.71-102.0/data:/opt/interproscan/data \
        -v $PWD/input:/input \
        -v $PWD/temp:/temp \
        -v $PWD/output:/output \
        interpro/interproscan:5.71-102.0 \
...
::
    ...
    singularity exec \
        -B $PWD/interproscan-5.71-102.0/data:/opt/interproscan/data \
        -B $PWD/input:/input \
        -B $PWD/temp:/temp \
        -B $PWD/output:/output \
