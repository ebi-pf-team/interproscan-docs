...
For the latest InterProScan release (5.75-106.0), data can be downloaded with the following command:
...
The data will be extracted to interproscan-5.75-106.0/data.
...
::
    ...
    docker run --rm \
        -v $PWD/interproscan-5.75-106.0/data:/opt/interproscan/data \
        -v $PWD/input:/input \
        -v $PWD/temp:/temp \
        -v $PWD/output:/output \
